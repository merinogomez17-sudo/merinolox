...
CREATE TABLE clients (
  ...
  email TEXT,
  phone TEXT,
  status TEXT CHECK (status IN ('contacto inicial', 'seguimiento', 'cotización', 'cierre', 'perdido')) DEFAULT 'contacto inicial',
...
CREATE POLICY "Admins ver todo y vendedores solo lo suyo - SELECT" 
...
CREATE POLICY "Admins ver todo y vendedores solo lo suyo - INSERT" 
...
CREATE POLICY "Admins ver todo y vendedores solo lo suyo - UPDATE" 
...
CREATE TABLE activities (
  ...
  vendedor_id UUID REFERENCES profiles(id) NOT NULL,
  type TEXT CHECK (type IN ('Llamada', 'Email', 'WhatsApp', 'Visita')) NOT NULL,
...
CREATE POLICY "Admins ver todo y vendedores solo lo suyo - SELECT actividades" 
...
CREATE POLICY "Insertar actividades" 
...
BEGIN
  ...
  VALUES (new.id, new.raw_user_meta_data->>'full_name', COALESCE(new.raw_user_meta_data->>'role', 'vendedor'));
  RETURN new;
END;
